...
- [x] P2.4 Confidence/coverage/applicability controls (backend surfaced in current UI)
  - AI analysis responses now include structured `analysis_controls` snapshot fields.
  - `Analyze with AI` narrative now renders a dedicated control summary section generated by backend.
  - Saved analysis reopen payload now preserves the same control snapshot for traceability.
- [x] P2.5 FRAT workspace (deterministic workflow + approval UX)
  - Added dedicated `/frat` page with backend-truth workflow:
    - select flight test
    - create/edit draft input snapshot
    - link analysis-job evidence
    - run deterministic scoring
    - visualize hard-stops and recommendation state
    - approve/reject/finalize state transitions
    - export finalized immutable FRAT PDF artifact
  - Added shared navigation entry (`FRAT Risk`) and frontend API contract support for `/api/frat/*`.
